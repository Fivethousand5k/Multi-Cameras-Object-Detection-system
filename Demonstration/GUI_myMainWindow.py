...
import sys
import random
from VideoPlayer2 import Player
...
'''
        ...
        self.Player1 = Player(self.mutex1)
        self.Player2 = Player(self.mutex2)
        self.Player3 = Player(self.mutex3)
        self.Player4 = Player(self.mutex4)



        self.Players_list=[self.Player1,self.Player2,self.Player3,self.Player4]
        self.mutex_list=[self.mutex1,self.mutex2,self.mutex3,self.mutex4]
    ...
    def init_fix_size(self):
        width, height = int(self.right_widget.width()/2), int(self.right_widget.height()/2)
        for player in self.Players_list:
            player.Fix_Size((width,height))
    def init_functions(self):
        video_src1 = 'offline_videos/1.mp4'
        video_src2 = 'offline_videos/2.mp4'
        self.Player1.display(video_src1)
        self.Player2.display(video_src2)
        self.Player3.display(video_src1)
        self.Player4.display(video_src2)
    ...
    def online(self):
        self.change_num_of_players(random.randint(1,4))  # 切换屏幕数量
...
#         self.tree.itemDoubleClicked.connect(self.start_online)
    ...
    def start_offline(self,address):
        ...
        for id, player in enumerate(self.Players_list):
            if not player.is_working:  # found a unoccupied player
                chosen_index = id
                break
        if chosen_index != -1:
            player = self.Players_list[chosen_index]
            player.play_new(address)
    ...
    def change_num_of_players(self,num):
        ...
        # fix videoplayers'size
        width, height = None, None
        if num == 1:
            width, height = int(self.right_widget.width()), int(self.right_widget.height())
        elif num == 2:
            width, height = int(self.right_widget.width() / 2), int(self.right_widget.height() )
        else:
            width, height = int(self.right_widget.width() / 2), int(self.right_widget.height() / 2 )
